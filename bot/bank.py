...
async def choose_country_from_constructor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query:
        # Сначала отвечаем на колбэк, чтобы кнопка не 'зависла' синей
        await query.answer()
        print(f"💳 [DEBUG] Переход в банки из конструктора. Data: {query.data}")
    
    # Вызываем отрисовку меню стран
    # ВАЖНО: мы вызываем её, чтобы она отредактировала сообщение и показала кнопки стран
    await choose_country(update, context)
    
    # Явно импортируем стейт (лучше делать это в начале файла, но можно и тут)
    from bot.states import SELECT_CATEGORY
    
    print(f"🔄 [DEBUG] Возвращаем стейт: SELECT_CATEGORY ({SELECT_CATEGORY})")
    
    # Возвращаем стейт Конструктору, чтобы он знал, что теперь мы в меню выбора стран
...
# D:\aRabota\TelegaBoom\030_mylinkspace\bot\bank.py
...
async def choose_country(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from bot.bankper import start_bank_config_flow  # Импорт нового движка
    from bot.states import SELECT_CATEGORY # Импортируем наш стейт 20
    ...
    # ⚡ ПРОВЕРКА: Если нажали конкретно Wise или Revolut — уходим в новый FLOW сразу
    if cb_data == "method_revolut":
        print("🚀 [DEBUG] Нажата кнопка Revolut, запускаем новый FLOW (стейт 282)")
        return await start_bank_config_flow(update, context, "revolut")
        
    if cb_data == "method_wise":
        print("🚀 [DEBUG] Нажата кнопка Wise, запускаем новый FLOW (стейт 282)")
        return await start_bank_config_flow(update, context, "wise")
    
    # Определяем способ ответа
    if update.callback_query:
        query = update.callback_query
        await query.answer()
        # Используем HTML для единообразия с остальным ботом
    ...
    keys_to_clear = [
        ...
        'current_field_index', 'flow_config', 'flow_step_index', 'flow_answers'
    ]
    for key in keys_to_clear:
        context.user_data.pop(key, None)
    
    text = (
        "🌍 <b>Как хотите получать переводы?</b>\n\n"
        "👇 <b>Выберите платежную систему или страну банка:</b>"
    )
    
    # ... (твои списки кнопок payment_wallet_buttons и bank_buttons остаются без изменений)
    payment_wallet_buttons = [
        ...
        InlineKeyboardButton("🇺🇿 Click", callback_data="method_click_uz"),
    ...
    print(f"🔄 [DEBUG] Меню отрисовано, возвращаем стейт: {SELECT_CATEGORY}")
    return SELECT_CATEGORY # Возвращаем 20, чтобы работали кнопки стран
...
async def method_paypal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Заполняем "топливо" для стейта 502
    method_id = "paypal"
    context.user_data['selected_methods_full'] = {
        method_id: {
            "name": "PayPal",
            "fields": ["link"], # Это имя поля, которое будет искать process_field_input
            "data": {}
        }
    }
    context.user_data['filling_queue'] = [method_id]
    context.user_data['current_field_index'] = 0
    
    # Чтобы process_field_input понимал, что мы в категории переводов
    context.user_data['category'] = 'transfers'

    text = (
       "💶 <b>PayPal</b>\n\n"
       "✏️ Введите ваш <b>username</b> или ссылку <code>paypal.me/name</code>\n\n"
       "👇 Жду ввод в поле чата..."
    )
    ...
    # Возвращаем 502. Конструктор его подхватит!
    print(f"DEBUG: Переходим в стейт {WAIT_PAYPAL}")
...
async def revolut_start_full(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Инициализирует процесс сбора полных реквизитов (Шаг 1)"""
    query = update.callback_query
    
    # 1. Безопасный ответ на колбэк
    if query:
        await query.answer()
        print(f"💳 bank.py получил кнопку: {query.data}")
    
    # 2. Очищаем/инициализируем данные для нового ввода
    context.user_data['revolut_data'] = {}
    context.user_data['category'] = 'transfers' # На всякий случай фиксируем категорию
    
    text = (
       "🏦 <b>Revolut — Шаг 1/6</b>\n\n"
       "✏️ Введите ваш Revolut <b>username</b> или ссылку.\n\n"
       "📌 Например:\n"
       "• <code>revolut.me/john123</code>\n"
       "• Или просто <b>john123</b>\n\n"
       "👇 <b>Жду ваш ввод в поле сообщения...</b>"
    )
    ...
    # 4. ВОЗВРАЩАЕМ ЧИСЛО 282 (чтобы ConversationHandler точно его узнал)
    # Если у тебя WAIT_REVOLUT_LOGIN определен как 282, можно оставить переменную,
    # но для надежности сейчас лучше вернуть число.
    return 282
...
async def process_revolut_login(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Чистит и сохраняет логин, переходит к Beneficiary (Шаг 2)"""
    # 1. Проверка на наличие сообщения (чтобы не упасть)
    if not update.message or not update.message.text:
       return 282 # Возвращаемся в WAIT_REVOLUT_LOGIN
    
    user_input = update.message.text
    # Печатаем текст сообщения для отладки вместо несуществующего callback_query
    print(f"💳 bank.py (Revolut Login) получил текст: {user_input}")
    ...
    # 4. Подготовка следующего шага
    text = (
       "🏦 <b>Revolut — Шаг 2/6</b>\n\n"
       "Введите <b>Beneficiary</b> (Имя получателя):\n\n"
       "📌 <b>Пример:</b>\n"
    ...
    # 5. Возвращаем стейт 283 (WAIT_REVOLUT_BENEFICIARY)
    return 283
...
async def process_revolut_quick_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если пришел текст (нормальный сценарий)
    if update.message:
        user_input = update.message.text
        # Чистим ввод от лишних пробелов и символа @
        login = user_input.strip().lstrip('@')
        
        if len(login) > 50:
            await update.message.reply_text("❌ Слишком длинный Revtag (макс. 50 символов)")
            return 288  # WAIT_REVOLUT_QUICK
        
        full_link = f"https://revolut.me/{login}"
        
        # Сохраняем данные для финальной записи в БД
        context.user_data['payment_data'] = {
            'type': 'revolut',
            'login': login,
            'link': full_link,
        ...
        print(f"✅ Revolut Quick: данные сохранены для {login}")
        
        await update.message.reply_text(
            f"✅ <b>Проверьте данные:</b>\n\n"
            f"Тип: <b>Revolut Quick</b>\n"
            f"Логин: <b>{login}</b>\n"
            f"Ссылка: {full_link}\n\n"
            f"👇 <b>Всё верно?</b>",
            reply_markup=InlineKeyboardMarkup([
        ...
        # Переходим в стейт подтверждения (CONFIRM_PAYMENT)
        return 270

    # Если вдруг юзер нажал кнопку вместо ввода текста
...
async def process_revolut_beneficiary(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет получателя и просит IBAN (Шаг 3/6)"""
    # 1. Проверка на наличие сообщения
    if not update.message or not update.message.text:
       return 283 # WAIT_REVOLUT_BENEFICIARY
    
    beneficiary = update.message.text.strip()
    
    # Печатаем текст сообщения для отладки (вместо падения на callback_query)
    print(f"💳 bank.py (Revolut Beneficiary) получил текст: {beneficiary}")
    ...
    # 3. Сохранение данных (проверяем наличие словаря)
    if 'revolut_data' not in context.user_data:
       context.user_data['revolut_data'] = {}
    ...
    # 5. Возвращаем стейт 284 (WAIT_REVOLUT_IBAN)
    return 284
...
async def process_revolut_iban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет IBAN и просит BIC/SWIFT (Шаг 4/6)"""
    # 1. Проверка на наличие сообщения
    if not update.message or not update.message.text:
       return 284 # WAIT_REVOLUT_IBAN
    
    iban = update.message.text.strip().upper() # Переводим в верхний регистр для порядка
    
    # Логируем текст вместо падения на .callback_query
    print(f"💳 bank.py (Revolut IBAN) получил текст: {iban}")
    ...
    # 5. Возвращаем стейт 285 (WAIT_REVOLUT_BIC)
    return 285
...
async def process_revolut_bic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет BIC и просит корреспондента (Шаг 5/6)"""
    # 1. Проверка на наличие сообщения
    if not update.message or not update.message.text:
       return 285 # WAIT_REVOLUT_BIC (текущий стейт)
    
    bic = update.message.text.strip().upper()
    print(f"💳 bank.py (Revolut BIC) получил текст: {bic}")
    
    # 2. Валидация
    if len(bic) > 50:
       await update.message.reply_text("❌ Слишком длинный BIC (макс. 50 символов)")
       return 285 # WAIT_REVOLUT_BIC
    ...
    text = (
       ...
       "📌 <b>Пример:</b>\n"
       "<code>CHASDEFX</code>\n\n"
       "👇 <b>Жду ваш ввод или нажмите «Пропустить»...</b>"
    )
    ...
    # 5. Возвращаем стейт 286 (WAIT_REVOLUT_CORRESPONDENT)
    return 286
...
async def process_revolut_correspondent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обрабатывает ввод корреспондента и переходит к адресу (Шаг 6/6)"""
    # 1. Защита от пустого сообщения
    if not update.message or not update.message.text:
       return 286 # WAIT_REVOLUT_CORRESPONDENT (текущий стейт)
    
    user_input = update.message.text.strip()
    print(f"💳 bank.py (Revolut Corr) получил текст: {user_input}")
    
    # 2. Валидация длины
    if len(user_input) > 100: # Увеличил лимит, так как BIC корреспондента может быть длиннее
       await update.message.reply_text("❌ Слишком длинный ввод (макс. 100 символов)")
       return 286 # WAIT_REVOLUT_CORRESPONDENT
    ...
    # 4. Подготовка финального шага ввода
    text = (
       "🏦 <b>Revolut — Шаг 6/6</b>\n\n"
       "Введите <b>адрес банка</b> (если есть):\n\n"
       "📌 <b>Пример:</b>\n"
       "<code>Konstitucijos ave. 21B, 08130 Vilnius, Lithuania</code>\n\n"
       "👇 <b>Жду ваш ввод или нажмите «Пропустить»...</b>"
    )
    ...
    # 5. Возвращаем стейт 287 (WAIT_REVOLUT_ADDRESS)
    return 287
...
async def process_revolut_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сохраняет адрес и показывает подтверждение (Финал ввода)"""
    # 1. Защита от пустого сообщения
    if not update.message or not update.message.text:
       return 287 # WAIT_REVOLUT_ADDRESS
    
    user_input = update.message.text.strip()
    print(f"💳 bank.py (Revolut Final) получил текст: {user_input[:20]}...")
    ...
    # 3. Сохраняем адрес, если это не команда пропуска
    if 'revolut_data' not in context.user_data:
        context.user_data['revolut_data'] = {}
    ...
    # 4. Подтягиваем все собранные данные
    data = context.user_data.get('revolut_data', {})
    login = data.get('login', '')
    full_link = f"https://revolut.me/{login}"
    
    # 5. Упаковка данных в основной словарь платежа для БД
    context.user_data['payment_data'] = {
       'type': 'revolut',
       'login': login,
    ...
    # 6. Формируем итоговый текст для проверки
    text = (
        "📊 <b>Проверьте ваши реквизиты:</b>\n\n"
        f"🔗 <b>Ссылка:</b> {full_link}\n"
        f"👤 <b>Получатель:</b> {data.get('beneficiary', '')}\n"
        f"🏦 <b>IBAN:</b> <code>{data.get('iban', '')}</code>\n"
        f"🔑 <b>BIC/SWIFT:</b> <code>{data.get('bic', '')}</code>\n"
    )
    
    if data.get('correspondent'):
       text += f"🔄 <b>Корр. банк:</b> {data.get('correspondent')}\n"
    if data.get('address'):
       text += f"📍 <b>Адрес банка:</b> {data.get('address')}\n"
    ...
    # 7. Возвращаем стейт 270 (CONFIRM_PAYMENT)
    return 270
...
async def revolut_ask_address_step(target):
    ...
    # Определяем, как отправить: через edit (если кнопка) или через reply (если текст)
    try:
        if hasattr(target, 'edit_message_text'):
            ...
            await target.message.reply_text(text, reply_markup=reply_markup, parse_mode='HTML')
        else:
            # Фолбэк на случай странных объектов
            print("⚠️ revolut_ask_address_step: не удалось определить метод отправки")
    except Exception as e:
        print(f"❌ Ошибка в revolut_ask_address_step: {e}")

    # Возвращаем числовой стейт 287 (WAIT_REVOLUT_ADDRESS)
    return 287
...
async def revolut_skip_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Безопасно получаем накопленные данные
    data = context.user_data.get('revolut_data', {})
    if not data:
        print("⚠️ revolut_skip_address: revolut_data пуст!")
        # Если данных нет, откатываем на выбор метода
        return await method_revolut(update, context)
    
    login = data.get('login', '')
    full_link = f"https://revolut.me/{login}"
    
    # 2. Финальная упаковка в payment_data для записи в базу
    context.user_data['payment_data'] = {
       'type': 'revolut',
       'login': login,
       'beneficiary': data.get('beneficiary', ''),
       ...
       'bic': data.get('bic', ''),
       'correspondent': data.get('correspondent', ''),
       'address': None, # Явно ставим None
       'link': full_link,
       'payment_type': 'revolut'
    }
    
    # 3. Формируем текст подтверждения (красивый и читаемый)
    text = (
        "📊 <b>Проверьте ваши реквизиты:</b>\n\n"
        f"🔗 <b>Ссылка:</b> {full_link}\n"
        f"👤 <b>Получатель:</b> {data.get('beneficiary', '')}\n"
    ...
    text += "📍 <b>Адрес:</b> <i>пропущен</i>\n"
    text += "\n👇 <b>Всё верно?</b>"
    
    # 4. Редактируем сообщение с кнопками подтверждения
    keyboard = [
        [
            InlineKeyboardButton("✅ Да, сохранить", callback_data="confirm_yes"),
            InlineKeyboardButton("❌ Нет, заново", callback_data="confirm_no")
    ...
    # 5. Возвращаем стейт 270 (CONFIRM_PAYMENT)
    return 270
...
def clean_revolut_login(text: str) -> str:
    ...
    # 1. Убираем пробелы и протоколы (http/https)
    text = text.strip().replace("https://", "").replace("http://", "")
    
    # 2. Список доменов для поиска логина
    revolut_domains = [
       "revolut.me/",
       "revolut.com/pay/",
       "revolut.com/",
    ...
    # 3. Ищем домен и забираем часть после него
    for domain in revolut_domains:
       if domain in text_lower:
          login = text.split(domain, 1)[-1]
          break
    
    # 4. Если домен не найден, но есть слэш, берем последнюю часть
    if login is None and "/" in text:
       login = text.split("/")[-1]
    
    # 5. Если всё еще None — значит прислали просто текст
    if login is None:
       login = text
    
    # 6. КРИТИЧЕСКИЙ ФИКС: Отрезаем параметры запроса (?...) и якоря (#...)
    # Это защитит от ссылок вида revolut.me/nick?src=copy
    login = login.split('?')[0].split('#')[0]
    
    # 7. Финальная чистка от мусора
    login = login.strip('/ ').lstrip('@')
    
    # Если на выходе пусто, возвращаем исходник без пробелов (крайний случай)
    return login if login else text.strip('/ @')
...
async def show_revolut_final(target, context: ContextTypes.DEFAULT_TYPE):
    """Генерирует финальное сообщение из всех собранных данных (Full Mode)"""
    # 1. Защита: если данных нет, не падаем
    data = context.user_data.get('revolut_data', {})
    if not data:
        print("⚠️ show_revolut_final: Данные revolut_data отсутствуют!")
        # Можно вернуть в начало выбора метода
        return 280 # WAIT_REVOLUT_METHOD

    login = data.get('login', '')
    full_link = f"https://revolut.me/{login}"
    
    # 2. Упаковываем всё в payment_data для финального сохранения в БД
    context.user_data['payment_data'] = {
       'type': 'revolut',
       'login': login,
       'beneficiary': data.get('beneficiary', ''),
    ...
    if data.get('correspondent'):
       text += f"\n🔄 <b>Корр. банк:</b> {data.get('correspondent')}"
    if data.get('address'):
       text += f"\n📍 <b>Адрес банка:</b> {data.get('address')}"
    ...
    try:
        ...
        print(f"❌ Ошибка в show_revolut_final: {e}")

    # 5. ВАЖНО: Мы НЕ делаем pop('revolut_data') здесь.
    # Очистим его в confirm_yes после записи в PostgreSQL.
    
    # Возвращаем числовой стейт 270 (CONFIRM_PAYMENT)
    return 270
...
async def revolut_skip_correspondent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пропускает шаг корреспондента и переходит к адресу (Шаг 6)"""
    query = update.callback_query
    if query:
        await query.answer()
        print(f"💳 bank.py (Revolut) Юзер пропустил Correspondent BIC")
    
    # 1. Инициализируем словарь, если его нет (защита от сбоев)
    if 'revolut_data' not in context.user_data:
        context.user_data['revolut_data'] = {}
    
    # 2. Явно помечаем, что данных нет
    context.user_data['revolut_data']['correspondent'] = None
    
    # 3. Переходим к следующему шагу через вспомогательную функцию
    # Она сама отправит текст и вернет стейт 287 (WAIT_REVOLUT_ADDRESS)
